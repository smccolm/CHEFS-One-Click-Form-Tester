...
async function stopRun() {
  ...
  try {
    ...
    if (!response || !response.ok) {
      throw new Error(response && response.error ? response.error : 'The run did not acknowledge the stop request.');
    }
    setMessage('Stopping run…');
    for (let attempt = 0; attempt < 10; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 100));
      await refreshStatus();
      const status = String(latestRun && latestRun.status || '').toLowerCase();
      if (!['initializing', 'scanning', 'filling', 'settling', 'validating', 'submitting', 'waiting'].includes(status)) {
        break;
      }
    }
  } catch (error) {
