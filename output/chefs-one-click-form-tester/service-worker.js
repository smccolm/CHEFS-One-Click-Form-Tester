...
async function stopRunInTab(tabId) {
  ...
  try {
    ...
    acknowledged = Boolean(response && response.ok);
    if (!acknowledged) {
      controllerError = response && response.error ? response.error : 'The content controller did not acknowledge Stop Run.';
    }
  } catch (error) {
    controllerError = error && error.message ? error.message : String(error);
  }
  ...
  run = await mutateRun(runId, (storedRun) => {
    ...
    storedRun.events.push({
      ...
      pass: storedRun.progress && storedRun.progress.pass || 0,
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    switch (message.type) {
      ...
      case 'STOP_RUN_IN_TAB':
        return { ok: true, ...(await stopRunInTab(message.tabId)) };
