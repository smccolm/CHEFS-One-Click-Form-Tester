...
(function installChefsTesterBridge() {
  ...
  function discoverFormInstances(wrapperId) {
    ...
    if (targetWrapper) {
      roots.push(...elementSearchRoots(targetWrapper));
    }

    for (const form of cachedForms) {
      if (isFormInstance(form)) {
        roots.push(form);
      }
    }
    ...
    const candidates = [...formElements, appElement, document.body, document.documentElement].filter(Boolean);
    for (const element of candidates) {
      roots.push(...elementSearchRoots(element));
    }
  ...
  function findComponentCandidates(key, wrapperId) {
    ...
    const add = (instance, formIndex) => {
      ...
      const ownsWrapper = Boolean(
        targetWrapper &&
        element &&
        (
          element === targetWrapper ||
          (targetWrapper.contains && targetWrapper.contains(element)) ||
          (element.contains && element.contains(targetWrapper))
        )
      );
      if (keyMatches || idMatches) {
        ...
        matches.push({
          ...
          score: (idMatches ? 1000 : 0) + (ownsWrapper ? 800 : 0) + (keyMatches ? 100 : 0) +
            (element && element.isConnected ? 10 : 0) - formIndex,
          ownsWrapper
        });
      }
    };
  ...
  async function selectOrgbookResult(payload) {
    ...
    const component = candidates.find((candidate) => {
      const type = String((candidate.component && candidate.component.type) || candidate.type || '').toLowerCase();
    ...
    const configuredUrl = String(component.component && component.component.data
      ? component.component.data.url || ''
      : '');
    ...
    if (component.itemsLoaded && typeof component.itemsLoaded.then === 'function') {
      await Promise.race([
    ...
    const selected = options.find((option) =>
      option && typeof option.value === 'string' && option.value.trim() === preferredValue
    );
    if (!selected) {
      return {
        applied: false,
