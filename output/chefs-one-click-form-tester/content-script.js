...
(function installChefsTesterContentScript() {
  ...
  function guidanceNumberBounds(value) {
    ...
    const amount = '((?:CAD\\s*)?(?:CA\\s*)?\\$?\\s*-?[0-9][0-9,]*(?:\\.[0-9]+)?)';
    const range = text.match(new RegExp(`(?:\\bfrom|\\bbetween)\\s+${amount}\\s+(?:to|and|through|-)\\s+${amount}`, 'i'));
    const maximum = text.match(new RegExp(`(?:cannot\\s+be\\s+greater\\s+than|must\\s+not\\s+exceed|no\\s+more\\s+than|maximum(?:\\s+(?:of|is))?|max\\.?|up\\s+to)\\s*[:=-]?\\s*${amount}`, 'i'));
    const minimum = text.match(new RegExp(`(?:cannot\\s+be\\s+less\\s+than|must\\s+be\\s+at\\s+least|no\\s+less\\s+than|minimum(?:\\s+(?:of|is))?|min\\.?|at\\s+least)\\s*[:=-]?\\s*${amount}`, 'i'));
  ...
  const GUIDANCE_DATE_PATTERN = '(January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?[,]?\\s+(\\d{4})';

  function parseGuidanceDate(value) {
    const match = String(value || '').match(new RegExp(GUIDANCE_DATE_PATTERN, 'i'));
    if (!match) {
      return null;
    }
  ...
  class ChefsTesterController {
    ...
    async start(startMessage) {
      ...
      this.executeRun().catch(async (error) => {
        ...
        if (this.stopRequested || (error && error.code === 'CHEFS_TESTER_STOPPED')) {
    ...
    fileUploadPending(wrapper) {
      ...
      )).filter((indicator) => isVisible(indicator));
    ...
    addressValuePresent(wrapper) {
      ...
      const controls = Array.from(wrapper.querySelectorAll('input:not([type="hidden"]), textarea'))
    ...
    async fillChoices(descriptor, searchRemote) {
      ...
      if (!chosen) {
        if (searchRemote) {
          ...
          if (keyboardPersisted) {
            ...
            return {
              success: true,
              valueInfo: safeValueInfo('selected', true)
            };
          }

          let bridgeResult = null;
          try {
            bridgeResult = await this.bridgeCommand('SELECT_ORGBOOK_RESULT', {
              key: descriptor.key,
              wrapperId: descriptor.wrapperId || (descriptor.wrapper && descriptor.wrapper.id) || '',
              query: 'wonderful',
              preferredValue: 'WONDERFUL FLOORING'
            }, 10000);
          } catch (error) {
            await this.log('REMOTE_AUTOCOMPLETE_BRIDGE_FALLBACK_FAILED', {
              componentId: descriptor.id,
              key: descriptor.key,
              componentType: descriptor.type,
          ...
          const bridgePersisted = bridgeResult && bridgeResult.applied
          ...
          await this.log('REMOTE_AUTOCOMPLETE_SELECTION_FAILED', {
            ...
            key: descriptor.key,
            componentType: descriptor.type,
            resultCount: bridgeResult && Number.isFinite(bridgeResult.resultCount) ? bridgeResult.resultCount : 0,
      ...
      wrapper = persisted || this.liveWrapper(descriptor) || wrapper;

      if (!persisted) {
        const search = wrapper.querySelector('.choices__input--cloned');
        if (search) {
          const retryQuery = searchRemote ? this.remoteAutocompleteQuery(descriptor) : chosenText;
          const retryControl = wrapper.querySelector('.choices[role="combobox"], .choices__inner');
          if (retryControl) {
            retryControl.click();
          }
          if (searchRemote) {
          ...
          persisted = await this.waitForChoiceValue(descriptor, 3000, 1200);
          wrapper = persisted || this.liveWrapper(descriptor) || wrapper;
        }
      }
    ...
    async fillRemoteAutocomplete(descriptor) {
      ...
      if (descriptor.type.includes('orgbook')) {
        let formioResult = null;
        try {
          formioResult = await this.bridgeCommand('SELECT_ORGBOOK_RESULT', {
            key: descriptor.key,
            wrapperId: descriptor.wrapperId || (descriptor.wrapper && descriptor.wrapper.id) || '',
            query: 'wonderful',
            preferredValue: 'WONDERFUL FLOORING'
        ...
        const formioPersisted = formioResult && formioResult.applied
          ? await this.waitForChoiceValue(descriptor, 3000, 1200)
          : null;
        if (formioPersisted) {
          await this.log('REMOTE_AUTOCOMPLETE_SELECTION_SUCCEEDED', {
            ...
            key: descriptor.key,
            componentType: descriptor.type,
            resultCount: Number.isFinite(formioResult.resultCount) ? formioResult.resultCount : null,
            selectionMethod: 'formio-select-lifecycle',
            formioRootCount: formioResult.lookup && Number.isFinite(formioResult.lookup.formRootCount)
              ? formioResult.lookup.formRootCount
              : null,
            formioComponentCount: formioResult.lookup && Number.isFinite(formioResult.lookup.inspectedComponentCount)
              ? formioResult.lookup.inspectedComponentCount
              : null,
            wrapperMatched: Boolean(formioResult.lookup && formioResult.lookup.wrapperMatched),
            persistenceMs: 1200
          });
          return {
            success: true,
        ...
        await this.log('REMOTE_AUTOCOMPLETE_FORMIO_SELECTION_FAILED', {
          ...
          key: descriptor.key,
          componentType: descriptor.type,
          resultCount: formioResult && Number.isFinite(formioResult.resultCount) ? formioResult.resultCount : 0,
          formioRootCount: formioResult && formioResult.lookup && Number.isFinite(formioResult.lookup.formRootCount)
            ? formioResult.lookup.formRootCount
            : null,
          formioComponentCount: formioResult && formioResult.lookup && Number.isFinite(formioResult.lookup.inspectedComponentCount)
            ? formioResult.lookup.inspectedComponentCount
            : null,
          wrapperMatched: Boolean(formioResult && formioResult.lookup && formioResult.lookup.wrapperMatched),
          reason: formioResult && formioResult.applied
      ...
      await delay(500);
      wrapper = this.liveWrapper(descriptor) || wrapper;
      const liveInput = Array.from(wrapper.querySelectorAll('input:not([type="hidden"]), textarea'))
    ...
    constraints(descriptor, control) {
      ...
      return {
        ...
        min: firstNumber(control && control.min !== '' ? control.min : null, meta.min, renderedNumberBounds.min),
        max: firstNumber(control && control.max !== '' ? control.max : null, meta.max, renderedNumberBounds.max),
    ...
    dateValuePlan(descriptor, attempt, control) {
      ...
      if (bounds.min && bounds.max) {
        date = new Date((/end|completion|finish/.test(context) ? bounds.max : bounds.min).getTime());
        date.setDate(date.getDate() + (/end|completion|finish/.test(context) ? -dayOffset : dayOffset));
      } else if (bounds.min) {
        date = new Date(bounds.min.getTime());
        date.setDate(date.getDate() + dayOffset);
      } else if (bounds.max) {
        date = new Date(bounds.max.getTime());
        date.setDate(date.getDate() - dayOffset);
      } else if (/birth/.test(context)) {
      ...
      if (type === 'datetime-local') {
        value = `${yyyy}-${mm}-${dd}T10:30`;
      } else if (type === 'time') {
        value = '10:30';
      } else if (type === 'date') {
        value = `${yyyy}-${mm}-${dd}`;
      } else if (/mm\/dd\/yyyy/.test(placeholder)) {
        value = `${mm}/${dd}/${yyyy}`;
      } else if (/dd\/mm\/yyyy/.test(placeholder)) {
        value = `${dd}/${mm}/${yyyy}`;
      } else {
        value = `${yyyy}-${mm}-${dd}`;
      }
      return { date, value, min: bounds.min, max: bounds.max };
    }
    ...
    async fillDateTime(descriptor, attempt) {
      ...
      const pickerInput = inputs.find((item) => item._flatpickr && typeof item._flatpickr.setDate === 'function');
      ...
      if (pickerInput) {
        try {
          ...
          await delay(180);
          wrapper = this.liveWrapper(descriptor) || wrapper;
          if (!this.inspectEmpty(wrapper, descriptor.type)) {
            await this.log('DATE_VALUE_APPLIED', {
              componentId: descriptor.id,
              key: descriptor.key,
              method: 'flatpickr',
              minDate: plan.min ? plan.min.toISOString().slice(0, 10) : null,
              maxDate: plan.max ? plan.max.toISOString().slice(0, 10) : null,
              value: safeValueInfo(value, true)
            });
            return { success: true, valueInfo: safeValueInfo(value, true) };
          }
      ...
      if (input) {
        ...
        if (!this.inspectEmpty(wrapper, descriptor.type)) {
          await this.log('DATE_VALUE_APPLIED', {
            componentId: descriptor.id,
            key: descriptor.key,
            method: input.readOnly ? 'readonly-dom-events' : 'dom-events',
            minDate: plan.min ? plan.min.toISOString().slice(0, 10) : null,
            maxDate: plan.max ? plan.max.toISOString().slice(0, 10) : null,
            value: safeValueInfo(value, true)
          });
    ...
    async uploadFileByDomDrop(descriptor, file) {
      ...
      const expectedBaseline = pending.baselineCount;
      ...
      while (Date.now() - pending.startedAt < CONFIG.uploadTimeoutMs) {
        ...
        const uploadPending = this.fileUploadPending(wrapper);
        if (!uploadPending && (
          namedRows.length ||
        ...
        if (now - lastPendingLogAt >= 10000) {
          ...
          await this.log('UPLOAD_STILL_PENDING', {
            ...
            key: descriptor.key,
            filename: file.name,
            pendingForMs: now - pending.startedAt,
      ...
      try {
        ...
        if (!/still pending/i.test(error.message || '')) {
          ...
          try {
            ...
            if (this.stopRequested || (apiError && apiError.code === 'CHEFS_TESTER_STOPPED')) {
              throw apiError;
            }
            bridgeError = new Error(`DOM drop failed: ${error.message} API fallback failed: ${apiError.message}`);
          }
        }
      }
    ...
    async handleGrid(descriptor) {
      ...
      while (rowCount < targetRows) {
        ...
        if (!button) {
          if (rowCount > 0) {
            if (descriptor.type.includes('editgrid') && this.editGridSaveButtons(wrapper).length) {
              await this.log('GRID_ROW_EDITOR_PENDING', {
                componentId: descriptor.id,
                key: descriptor.key,
                componentType: descriptor.type,
                existingRows: rowCount,
    ...
    async commitOpenEditGridRows() {
      ...
      for (let guard = 0; guard < 5; guard += 1) {
        ...
        const gridId = grid && grid.id ? grid.id : '';
    ...
    stop() {
      ...
      Promise.resolve().then(() => this.finishStopped()).catch(() => {});
